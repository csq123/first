	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				window.location.href=path+"/sys/bill/billlist.html/";
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/sys/bill/billview/"+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/bill/billmodify.html?billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
