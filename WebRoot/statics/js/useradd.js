...
$(function(){
	...
	userRole.next().html("*");
		...
		if(errorinfo.val()==null||errorinfo.val()==""){
			a_idPaicPath.next().html("* 上传文件大小不能超过500kb *上传文件类型必须为：jpg,jpeg,png,pneg");
		}else{
			a_idPaicPath.next().html(errorinfo.val());
		}
		if(errorinfo_wp.val()==null||errorinfo_wp.val()==""){
			workPicPath.next().html("* 上传文件大小不能超过500kb *上传文件类型必须为：jpg,jpeg,png,pneg");
		}else{
			workPicPath.next().html(errorinfo_wp.val());
		}
